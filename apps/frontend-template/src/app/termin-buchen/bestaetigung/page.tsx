...
interface AppointmentRow {
  ...
  salon_id: string;
  staff: AppointmentStaffRow | AppointmentStaffRow[] | null;
  appointment_services: AppointmentServiceRow[] | null;
}

function getAppointmentStaff(staff: AppointmentStaffRow | AppointmentStaffRow[] | null): AppointmentStaffRow | null {
...
export default async function BookingConfirmationPage({ searchParams }: PageProps) {
  ...
  if (appointmentId) {
    ...
    appointment = (data || null) as AppointmentRow | null;
  ...
  const bookingNumber = params.nr || (appointment?.id ? `SW-${appointment.id.slice(0, 8).toUpperCase()}` : 'SW-OFFEN');
  const staffMember = appointment ? getAppointmentStaff(appointment.staff) : null;
  ...
  const displayData = appointment
    ? {
        ...
        staff: staffMember?.display_name || 'Ihr Stylist',
        services: appointment.appointment_services?.map((s) => s.service_name).filter(Boolean) || [],
