...
export default async function HomePage() {
  ...
  return (
    <>
      ...
      <ReviewsCarouselSection salonCity={salon?.city} feedback={feedbackData} />
...
function ReviewsCarouselSection({ salonCity, feedback }: ReviewsCarouselSectionProps) {
  const cityName = salonCity || 'St. Gallen';

  // Fallback reviews if no approved feedback yet
  const fallbackReviews = [
    {
      id: 'fallback-1',
      name: 'Sarah M.',
      rating: 5,
      comment: `Absolut bester Friseursalon in ${cityName}! Das Team ist super freundlich und das Ergebnis immer perfekt.`,
    },
    {
      id: 'fallback-2',
      name: 'Thomas K.',
      rating: 5,
      comment: 'Professionelle Beratung und erstklassiges Handwerk. Hier fühlt man sich wirklich gut aufgehoben.',
    },
    {
      id: 'fallback-3',
      name: 'Nina B.',
      rating: 5,
      comment: 'Meine Balayage ist fantastisch geworden! Kann BeautifyPRO nur empfehlen.',
    },
    {
      id: 'fallback-4',
      name: 'Michael R.',
      rating: 5,
      comment: 'Seit Jahren mein Stammfriseur. Qualität und Service sind unschlagbar!',
    },
    {
      id: 'fallback-5',
      name: 'Lisa W.',
      rating: 5,
      comment: 'Die Beratung war top und das Ergebnis noch besser. Sehr empfehlenswert!',
    },
    {
      id: 'fallback-6',
      name: 'David H.',
      rating: 5,
      comment: 'Tolle Atmosphäre und professionelles Team. Komme immer gerne wieder.',
    },
  ];

  // Use database feedback if available (top 6), otherwise fallback
  const reviews = feedback.length > 0 ? feedback.slice(0, 6) : fallbackReviews;
