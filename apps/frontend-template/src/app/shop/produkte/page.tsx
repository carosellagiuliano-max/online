...
export const metadata: Metadata = {
  title: 'Alle Produkte',
  description: 'Entdecken Sie alle Produkte im Online-Shop.',
};
...
async function getProducts(searchParams: {
  ...
  if (!supabase) {
    ...
    return { products: [], categories: [], total: 0, page: 1, limit: 12 };
  }
  ...
  if (error) {
    ...
    return { products: [], categories: [], total: 0, page, limit };
  }
  ...
  // Transform products
  const products = ((data || []) as ProductListRow[]).map((p) => {
    const images = p.product_images || [];
    const primaryImage = images.find((img) => img.is_primary) || images[0];
    const category = getCategory(p.product_categories);
    return {
      id: p.id,
      name: p.name,
      slug: p.slug,
...
export default async function ProductsPage({
  ...
  const priceOptions = [
    ...
    { label: 'Unter CHF 25', value: '0-25' },
    { label: 'CHF 25 - 50', value: '25-50' },
    { label: 'CHF 50 - 100', value: '50-100' },
    { label: 'Über CHF 100', value: '100-9999' },
  ];
  ...
  const currentPriceFilter = params.minPrice && params.maxPrice
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-1">
                  {priceOptions.map((option) => {
                    const [min, max] = option.value === 'all' ? [undefined, undefined] : option.value.split('-');
                    const isActive = currentPriceFilter === option.value ||
                      (option.value === 'all' && !params.minPrice && !params.maxPrice);
                    return (
                      <Link
                        key={option.value}
